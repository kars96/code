...
char *message = s; 
GLUquadricObj *quad;
GLfloat angle, angle1,outwicangle=-90.0,outbailangle=0.0,outbailx=1.57,outbaily=.12,outwicz=0.5;;
static GLfloat move=0.0,moves=-0.3,a=0,b=0,movef=-0.3;
...
void front()
{
  ...
  glClearColor (r, g, bl, fog);
	...
	glLoadName(TEXTID);
	drawtext(-1.0,-0.55,0.0,0.0004,"      BY : POOJA");

	
	//glLoadName(TEXTID);
	//drawtext(-1.0,-0.6,0.0,0.0004,"USN : 1EW13CS093");

    glLoadName(TEXTID);
	drawtext(-1.0,-0.7,0.0,0.0004,"      BY : POOJITHA.S");

	
	//glLoadName(TEXTID);
	//drawtext(-1.0,-0.75,0.0,0.0004,"USN : 1EW13CS097");

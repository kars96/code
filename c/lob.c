...
void detail()
{
	...
	glPushMatrix();
	glScalef(.25,.25,.25);
	strokeText("                    J.S.S ACADEMY OF TECHNICAL EDUCATION",0,0);
	glPopMatrix();
	// glFlush();
	glTranslatef(0,-100,0);
	glPushMatrix();
	glScalef(.25,.25,.25);
	strokeText("               DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING",0,0);
	glPopMatrix();
	glTranslatef(0,-100,0);
	glPushMatrix();
	glScalef(.25,.25,.25);
	strokeText("                                  BRIDGE",0,0);
	glPopMatrix();
	...
	glPushMatrix();
	glScalef(.25,.25,.25);
	strokeText("                                  put other details",0,0);
	glPopMatrix();*/
	glFlush();
	glutSwapBuffers();
}
void detailkb(unsigned char key,int x,int y)
{
	if(key)
	{
...
glutCreateMenu(main_menu);
	...
	glutDisplayFunc(detail);
	// glutDisplayFunc(display);
	
	myinit();
	glClearColor (.0, .0, 01.0, 1.0);
	glutMainLoop();
        return 0;

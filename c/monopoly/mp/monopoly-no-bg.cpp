...
public:
	...
	friend void result();friend void resultm(int ,int , int, int);
...
void choicem(int b,int s,int x,int y)
{
	...
	glutKeyboardFunc(spinnerkb2);glutSetWindow(swid2);
	glutPostRedisplay();
...
void choicekb(int k, int x,int y)
{
	...
	glutSpecialFunc(NULL);glutSetWindow(swid2);
	glutPostRedisplay();
...
void casino()
{
	...
	glLoadIdentity();
	gluOrtho2D(0,(resx/2)-(10*CM2PIX/2),0,(resy/2)-(5*CM2PIX/2));glMatrixMode(GL_MODELVIEW);
	glutSetWindow(swid3);
...
/*
void myText(const char *s,int  an)
{
...
void myTextCaps(int* arr,int  an)
{
	...
	d1(arr[0],9);
	glPopMatrix();
	glPushMatrix();	glTranslatef(47,0,0);for(int i=1;i<an;i++)
	{
		if(arr[i]!=-1){
		glPushMatrix();
		if(arr[i]==A | arr[i]==V | arr[i]==W | arr[i]==M | arr[i]==Y)
...
void getNames()
{
	...
	else
	{
	 ...
	 glPushMatrix();
	 glTranslatef(830,WH-548,0);glScalef(.7,.7,1);
	glColor3f(1,1,1);
	d1(BUTTON,9);
	glPopMatrix();
	glPushMatrix(); glTranslatef(830+10+10+5,WH-548+5,0);glColor4f(TH_COLOR,.75);
	rasterText("Ok",0,0);
	glPopMatrix();
...
{	startFlag=1;
	...
	glRotatef(-90,0,0,1);
	glScalef(.035,.035,.035);
	glColor3f(0,0,0);strokeText(brd->b[i].name,0,0);glPopMatrix();
	glPushMatrix();
	...
	glScalef(.035,.035,.035);
	glColor3f(0,0,0);strokeText(brd->b[i].name,0,0);
	glTranslatef(0,10,0);glPopMatrix();
	glPushMatrix();
	glTranslatef(brd->b[i].posv[0][0]-5,brd->b[i].posv[0][1]-20+10,0);
	glRotatef(-180,0,0,1);
	glScalef(.035,.035,.035);
	...
	glScalef(.035,.035,.035);
	glColor3f(0,0,0);strokeText(brd->b[i].name,0,0);
	glTranslatef(10,0,0);glPopMatrix();
	glPushMatrix();
	glTranslatef(brd->b[i].posv[0][0]-20+10,brd->b[i].posv[0][1]+5,0);
	glRotatef(-270,0,0,1);
	glScalef(.035,.035,.035);
...
glMaterialfv(GL_FRONT_AND_BACK,GL_SHININESS,mat_shininess);
	...
	glDisable(GL_LIGHT0);		glPopMatrix();
		}
...
glMaterialfv(GL_FRONT_AND_BACK,GL_SHININESS,mat_shininess);
	...
	glColor3f(.1,1,1);
	glTranslatef(200,300,1);glRotatef(90,1,0,0);
	glRotatef(0,0,1,0);glPopMatrix();
	static int z=1;
	if(z)
	{z=0;
}
...
//p1 end
	...
	//p1 pawn
	glEnable(GL_LIGHTING);
	glEnable(GL_LIGHT0);if(brd->me->pid==brd->p[0].pid)
	{
...
glLightfv(GL_LIGHT0,GL_DIFFUSE,lightint);
	glPushMatrix();
	glTranslatef(50,470,100);glRotatef(-80,1,0,0);
	glTranslatef(0,-10,0);glColor3f(1,0,0);glutSolidCone(20,60,40,40);
	glPopMatrix();
	glPushMatrix();
	...
	glColor4f(1,1,1,.5);glPushMatrix();
	glColor4f(1,1,1,.5);
	glBegin(GL_POLYGON);
	glVertex2f(10,50);
	glVertex2f(291,50);
...
//p2 pawn
	glColor3f(1,0,0);
	glEnable(GL_LIGHTING);
	glEnable(GL_LIGHT0);if(brd->me->pid==brd->p[1].pid)
	{
...
int containsPoint(int x1,int y1,int x2,int y2,int x,int y)
{
	...
	x1*=sx,x2*=sx,y1*=sy,y2*=sy;if(x>=x1 && x<=(x2) && y>=y1 && y<=(y2)) return 1;
	else return 0;
}
...
{	
	...
	strokeText2("Your progress will not be saved",0,0);glPopMatrix();
	glPushMatrix();
	glTranslatef(7*CM2PIX,4*CM2PIX,0);
	glScalef(2.5,3,3);
	glColor4f(TH_COLOR,.75);
...
glLightfv(GL_LIGHT0,GL_DIFFUSE,lightint);
	...
	glScalef(2,2,2);glRotatef(-80,1,0,0);
	glRotatef(25,0,0,1);
	glTranslatef(0,-10,0);glColor3f(1,0,0);glPushMatrix();
	 glRotatef(80,0,0,01);
	 glRotatef(180,1,0,0);
	 glTranslatef(0,0,-60);
	...
	glScalef(2.5,2.5,1);
	 glutSolidCube(10);
	glPopMatrix();
	glTranslatef(0,0,70);glutSolidSphere(15,20,20);
	glPopMatrix();
	glPopMatrix();
	glPushMatrix();
	glColor3f(1,0,0);
	glTranslatef(300,400,60);
	glRotatef(180,0,1,0);glPopMatrix();
	glDisable(GL_LIGHT0);
	glDisable(GL_LIGHTING);
	...
	glPopMatrix();
	 ...
	 glPushMatrix();
	 glTranslatef(830,WH-548-10,0);glScalef(.7,.7,1);
	glColor3f(1,1,1);
	d1(BUTTON,9);
	glPopMatrix();

	glPushMatrix(); glTranslatef(830+10+10+5,WH-546-10+3,0);glColor4f(TH_COLOR,.75);
	rasterText("Ok",0,0);
	glPopMatrix();
...
glLightfv(GL_LIGHT0,GL_DIFFUSE,lightint);
	...
	glRotatef(90,1,0,0);
	glRotatef(90,1,0,0);
	glTranslatef(0,0,-60);glPopMatrix();glPushMatrix();
	glScalef(1.9,1.9,.2);glPopMatrix();
	glPushMatrix();
	glTranslatef(0,0,-1.2);
	glScalef(2.1,2.1,.2);glPopMatrix();
	glPushMatrix();
	glTranslatef(0,0,-2.4);
	glScalef(2.5,2.5,1);glPopMatrix();
	glTranslatef(0,0,70);glPushMatrix();
	glRotatef(90,1.0f,0.0f,.0f);
	glRotatef(45,0,0,1);
	glTranslatef(0,0,-.6);glPushMatrix();glScaled(2.9,.4,0.4);
	glTranslatef(0.0,0.0,16.0);glPopMatrix();glPushMatrix();
	glScaled(.4,2.9,0.4);
	glTranslatef(0.0,0.0,16.0);glPopMatrix();
	glPopMatrix();
...
void mainScreen()
{
	int k=0;
	int i,d=0,j;
	int factor=-resx/10;glClearColor (1,1.0,1.0,1.0);
    glClear (GL_COLOR_BUFFER_BIT |GL_DEPTH_BUFFER_BIT);
    glLoadIdentity();
...
ccode[(int)':']=COLON;
	...
	time_t sec;
	time(&sec);
	srand((unsigned int)sec);	glShadeModel(GL_SMOOTH);
	f=fopen("LB.txt","r");
 ...
 for(int i=1;i<=9;i++)
	...
	glColor4f(TH_COLOR,.75);glBegin(GL_POINTS);
	glVertex2f(0,600);
	glVertex2f(resx,600);
	glVertex2f(resx,700);
	glVertex2f(0,700);
	glEnd();glColor3f(1,1,1);glPushMatrix();
	glTranslatef(10,610,0);
	d1(ARROW,9);
	glPopMatrix();
	glEndList();

	glNewList(SPIN,GL_COMPILE);glColor3f(1,1,1);
	glPushMatrix();
	glScalef(.9,.9,.9);
	glPushMatrix();
	glScalef(1.05,1.05,1.5);
	...
	glColor3f(1,1,1);for(i=-10,k=ONE;k<=NINE;k++,i-=30)
	{
	glPushMatrix();
	glRotatef(i,0,0,1);
...
void init()
{
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	glOrtho(0,resx,0,resy,-resy,resy);glMatrixMode(GL_MODELVIEW);
}
void init2()
{glClearColor(1,1,1,1);
